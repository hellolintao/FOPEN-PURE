...
Privacy rule: cache pair/member ids and aggregate match records; resolve display labels, avatar URLs, and public visibility from current `members` data in `analytics-engine.getPairAnalytics` / `analytics-engine.getPlayerAnalytics` read paths so revoke/delete flows cannot leave stale pair or opponent names in the UI.
