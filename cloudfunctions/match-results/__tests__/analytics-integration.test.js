...
test('buildBatchCtx.afterSettlement continues later season refreshes when first season analytics throws', async () => {
  const calls = []
  const ctx = buildBatchCtx({
    _id: 'admin1',
    openid: 'openid_admin',
    ...
    callFunction: async ({ name, data }) => {
      calls.push({ name, data })
      if (name === 'analytics-engine' && data.seasonId === 'season_2026') {
  ...
  const result = await ctx.afterSettlement({ successIds: ['m1', 'm2'], requestId: 'req_multi_season_partial_failure' })
